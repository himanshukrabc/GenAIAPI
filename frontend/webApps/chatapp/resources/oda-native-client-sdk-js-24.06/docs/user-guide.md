...
| **`enableVoiceOnlyMode`** | `false` | Enables the voice-only mode. Activating the voice-only mode eliminates the need for users to click the mic button each time after they receive a bot message. With this mode enabled, the SDK automatically initiates voice recognition once the TTS (Text-to-Speech) for the received message has been spoken out. |
...
| **`embeddedVideo`** | `true` | Embeds YouTube, Daily Motion, Vimeo, and Oracle Video Hub video links in messages when set `'true'`. These videos are always embedded in the video attachment messages. | 
